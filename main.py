...
Original file is located at
    https://colab.research.google.com/drive/1YVd2QTO_hvAkOcxcZF8EPFQjVvuE__lT
"""
...
SUPER_SHOOT_TIME = 30.0  # 新增：超级射击（SS）持续时间为 30 秒
LASER_DURATION = 0.08
...
class Player:
    def __init__(self, x, y):
        ...
        self.super_timer = 0  # 新增：超级射击（SS）剩余时间计时器
    ...
    def update(self, dt):
        ...
        self.super_timer = max(0, self.super_timer - dt)  # 更新超级射击计时器
    ...
    def current_damage(self):
        ...
        # 如果处于超级射击（SS）状态，伤害倍率变为3倍，否则为1倍
        super_mult = 3.0 if self.super_timer > 0 else 1.0
        return (1 + damage_level) * super_mult

    def current_cooldown(self):
        # 如果处于超级射击（SS）状态，基础攻速变为3倍，否则为1倍
        fire_rate = 3.0 if self.super_timer > 0 else 1.0
    ...
    def apply_powerup(self, powerup_type):
        if powerup_type == "size":
            ...
            # 获得或刷新超级射击（SS）的持续时间为 30 秒
            self.super_timer = SUPER_SHOOT_TIME
...
# --- 敌方弹幕子弹类 ---
class EnemyBullet:
    def __init__(self, center, direction, size=12, speed=280, damage=10):
        self.rect = pygame.Rect(0, 0, round(size), round(size))
        self.rect.center = center
        self.direction = pygame.Vector2(direction)
        ...
        self.speed = speed
        self.damage = damage
        self.color = (255, 140, 0)  # 橙色弹幕子弹，区别于玩家的蓝色子弹
    ...
    def draw(self, surface):
        # 绘制橙色发光效果的子弹
        pygame.draw.rect(surface, self.color, self.rect, border_radius=4)
        pygame.draw.rect(surface, (255, 215, 0), self.rect, width=2, border_radius=4)
...
class Enemy:
    def __init__(self, x, y, hp, is_ranged=False):
        self.rect = pygame.Rect(x, y, ENEMY_SIZE, ENEMY_SIZE)
        self.hp = hp
        self.attack_timer = 0
        self.is_ranged = is_ranged
        # 远程敌人的发射冷却定时器，初始化为一个随机值（1.0 ~ 5.0），防止多个敌人同一时间瞬间集火
        self.shoot_timer = uniform(1.0, 5.0)
    ...
    def update(self, dt, player, enemy_bullets):
        ...
        if self.is_ranged:
            ...
            if self.shoot_timer <= 0:
                shoot_dir = pygame.Vector2(player.rect.center) - pygame.Vector2(self.rect.center)
                if shoot_dir.length_squared() > 0:
                    enemy_bullets.append(EnemyBullet(self.rect.center, shoot_dir))
                self.shoot_timer = 5.0  # 重置攻击间隔为 5 秒
    ...
    def draw(self, surface):
        if self.is_ranged:
            # 远程敌人：紫金色（紫色身体，黄色高亮描边）
            pygame.draw.rect(surface, (147, 51, 234), self.rect, border_radius=7)
            pygame.draw.rect(surface, (234, 179, 8), self.rect, width=3, border_radius=7)
        else:
            # 普通近战敌人：红黑色
...
def spawn_enemies(kill_count, use_base_count=False):
    ...
    # 在这波敌人中，随机挑一个敌人获得远程弹幕能力
    ranged_enemy_index = randint(0, enemy_count - 1) if enemy_count > 0 else -1
    ...
    for i in range(enemy_count):
        ...
        is_ranged = (i == ranged_enemy_index)
        enemies.append(Enemy(x, y, enemy_hp, is_ranged))
    return enemies
...
def draw_hud(surface, font, player, enemy_count, kill_count, next_target, safe_break_timer, in_safe_zone):
    ...
    # 动态构建超级射击计时器显示 (SS)
    super_text = f"Super Shoot {player.super_timer:.0f}s" if player.super_timer > 0 else "Super Shoot 0s"
    ...
    # 动态展示超级射击倍率：SS处于激活期时显示 x3，否则为 x1
    current_super_dmg_mult = 3 if player.super_timer > 0 else 1
    current_super_fr_mult = 3 if player.super_timer > 0 else 1

    upgrades = (
        f"Bullet +{player.bullet_size_bonus}    "
        f"Speed +{player.bullet_speed_bonus}    "
        f"Damage +{player.damage_bonus}    "
        f"Super damage x{current_super_dmg_mult}    "
        f"Super fire x{current_super_fr_mult}    "
        f"{super_text}"
    )
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                shots.clear()
                enemies.clear()
                enemy_bullets.clear()  # 确保重开游戏时清空敌方弹幕
                powerups.clear()
                spawn_timer = 0
                powerup_timer = uniform(POWERUP_SPAWN_MIN, POWERUP_SPAWN_MAX)
                weapon_timer = uniform(WEAPON_SPAWN_MIN, WEAPON_SPAWN_MAX)
        ...
        if player.hp > 0:
            ...
            for e_bullet in enemy_bullets[:]:
                ...
                elif e_bullet.rect.colliderect(player.rect):
            ...
            for enemy in enemies[:]:
                # 传入 enemy_bullets，允许远程敌人添加攻击弹幕
                enemy.update(dt, player, enemy_bullets)
            ...
            if kill_count >= next_kill_target:
                enemies.clear()
                shots.clear()
                enemy_bullets.clear()  # 关卡过渡时，清空敌方的多余子弹
